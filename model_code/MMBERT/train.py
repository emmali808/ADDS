...
if __name__ == '__main__':
    ...
    parser.add_argument('--run_name', type = str, default="dataset", help = "run name for model")
    parser.add_argument('--data_dir', type=str, required=False,
                        default="dataset", help="path for data")
    parser.add_argument('--model_dir', type=str, required=False,
                        default="/home/viraj.bagal/viraj/medvqa/Weights/roco_mlm/val_loss_3.pt",
    ...
    parser.add_argument('--hidden_size', type=int, required=False, default=768, help="hidden size")  # 312
    ...
    scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, patience=args.patience, factor=args.factor, verbose=False)
    ...
    for epoch in range(args.epochs):
        ...
        train_loss, _, _, _, _ = train_one_epoch(trainloader, model, optimizer, criterion, device, scaler, args,
                                                 idx2ans)
        val_loss, predictions, val_acc, val_bleu = validate(valloader, model, criterion, device, scaler, args, val_df,
                                                            idx2ans)
    ...
    res=test(testloader, model, criterion, device, scaler, args, test_df, idx2ans)
    print(res)