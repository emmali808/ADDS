...
###
from graph_models_cluster import EHROntologyModel,ConcatEmbeddings,OntologyEmbedding
### 文件名字修改
from graph_models_hxj import GRAMModel,DRMMEHROntologyModel
...
def load_dataset(train_processed_file,valid_processed_file,test_processed_file,train_input_file,batch_size,dtype,add_admission):
    ...
    test_ehr_file = base_dir+'test_admissions.csv'#(tokenizer,processed_file,ehr_file,batch_size,dtype='no_onto',oper_type='normal')
    ## 自己修改的代码
    test_dataloader,_ ,_ = construct_query_dataloder(tokenizer,test_processed_file,test_ehr_file,batch_size = batch_size,dtype=dtype)
    ...
    ## gram模型
    train_input_dataloader,_ ,_ = construct_query_dataloder(tokenizer,train_input_file,train_ehr_file,batch_size = batch_size,dtype=dtype)
...
def test(model,test_dataloader,cohorts,device,dtype='cluster',train_dataloader=None,train_cohorts=None,random_state=0):
    ...
    for batch in tqdm(test_dataloader):
        with torch.no_grad():
            ...
            # outputs = model(left_x=batch.x.to(device),left_graph_index=batch.edge_index.long().to(device),left_x_batch=batch.batch.to(device))
            ## gram模型
            outputs = model(left_x=batch.x_left.to(device),left_graph_index=batch.edge_index_left.long().to(device),right_x=batch.x_right.to(device),right_graph_index=batch.edge_index_right.long().to(device),left_x_batch=batch.x_left_batch.to(device),right_x_batch=batch.x_right_batch.to(device),left_diag_cnt = batch.left_diag_cnt.to(device), right_diag_cnt = batch.right_diag_cnt.to(device),)
            all_embs.append(outputs.detach().cpu().numpy())
