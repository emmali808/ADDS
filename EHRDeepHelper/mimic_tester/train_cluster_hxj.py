...
#         print('picture save done')
    ...
    if args.do_train:
        ...
        total_eval_time=datetime.timedelta(0)
        total_test_time=datetime.timedelta(0)
        ...
        for _ in trange(int(args.num_train_epochs),desc='training epoch'):
            ...
            all_p_labels = []
            all_p_preds = []
            name=["epoch","training time","evaluating time","test time"]
            ...
            total_training_time +=(datetime.datetime.now() - start)
            print("training time = {}, loss = {}".format(
                total_training_time // 60,
                tr_loss/nb_tr_steps
            ))
            sub_list.append(total_training_time.total_seconds())
            ...
            if args.do_eval:
                ...
                total_eval_time +=(datetime.datetime.now() - start)
                print("evaluating time = {}, loss = {}".format(
                    total_eval_time // 60,
                    val_loss/nb_val_steps
                ))                
                sub_list.append(total_eval_time.total_seconds())
            ...
            if args.do_test:
                ...
                total_test_time +=(datetime.datetime.now() - start)
                print("test time = {}, the best metric: {}, nmi:{}, me_ri: {} ".format(
                    total_test_time // 60,
                    best_me_purity,
                    best_me_nmi,
                    best_me_ri
                ))
                sub_list.append(total_test_time.total_seconds())   
                print()
        ...
        output_time_file=pd.DataFrame(columns=name,data=total_list)
        #index=output_time_file.index
        #output_time_file.drop(index, inplace=True)        
        output_time_file.to_csv('time.csv')     
